...
import { getEngineInput, getUnderwritingRunState } from "@/lib/underwriting.functions";
...
const engineInputQ = (pid: string) =>
  queryOptions({
    queryKey: ["engine-input", pid],
    queryFn: () => getEngineInput({ data: { project_id: pid } }),
  });
...
const SCENARIO_LABELS: Record<string, string> = {
  base: "Base Case",
  revenue_down: "Revenue −10%",
  cost_overrun: "Cost +10%",
  rate_shock: "Rate +150bps",
  cap_expansion: "Cap +75bps",
...
export function AnalysisPanel({ projectId }: { projectId: string }) {
  ...
  if (!decision.hasUnderwriting || runState.freshness === "blocked") {
  ...
  return (
    <div className="space-y-6">
      ...
      >
        ...
        >
          {runState.freshness === "stale" ? "Outputs stale" : "Outputs current"}
        </Badge>
        {runState.latest_completed_run && (
          <Badge variant="outline" className="text-[11px]">
            Run version v{(runState.latest_completed_run as { run_number?: number }).run_number}
          </Badge>
        )}
        <Badge variant="outline" className="text-[11px]">
      ...
      <section>
        <div className="flex items-center gap-2 mb-3">
          ...
          <Eyebrow>What breaks the deal · stress matrix</Eyebrow>
        </div>
        ...
        <p className="text-xs text-muted-foreground mt-2">
          Red cells fall below the underwriting gate: these are the conditions under which the deal
          stops clearing.
        </p>
      </section>
      ...
      <div className="grid lg:grid-cols-3 gap-4">
        <Card className="p-5 elevated">
          <div className="flex items-center gap-2">
            ...
            <SectionLabel>Driver Analysis</SectionLabel>
          </div>
        ...
        <Card className="p-5 elevated">
          <div className="flex items-center gap-2">
            <Activity className="size-4 text-primary" />
            <SectionLabel>Stress worst-case</SectionLabel>
          </div>
          <p className="text-xs text-muted-foreground mt-2">
            Worst observed value across all stress runs:
          </p>
        ...
        <Card className="p-5 elevated">
          <div className="flex items-center gap-2">
            ...
            <SectionLabel>Covenant Analysis</SectionLabel>
          </div>
      ...
      <section>
        ...
        <div className="mt-3">
          <UnderwritingPanel projectId={projectId} />
        </div>
      </section>
    </div>
  );
    ...
    <div>
      ...
      <ol className="mt-2 space-y-1.5">
        {drivers.slice(0, 5).map((d) => (
          <li key={d.rank} className="text-xs">
            <span className="num text-muted-foreground mr-1.5">{d.rank}.</span>
