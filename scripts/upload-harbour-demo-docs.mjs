...
const DOCS = [
  {
    ...
    lines: [
      "Common equity contribution: $50,000,000",
      "Residential rental units: 220 units",
      "Retail area: 18,000 SF",
      "Office area: 32,000 SF",
    ],
  },
  {
    ...
    lines: [
      "Residential rent: $3,050 per unit per month",
      "Residential occupancy: 96%",
      "Retail rent: $42 per square foot",
      "Retail occupancy: 92%",
      "Office rent: $36 per square foot",
      "Office occupancy: 85%",
  ...
  },
  {
    ...
    lines: [
      "Senior loan amount: $162,500,000",
      "Interest rate: 6.25%",
      "Amortization: 30-year",
      "Minimum DSCR covenant: 1.20x",
      "Lender stabilization requirement: 93%",
...
const { data: projects, error: pErr } = await supabase
  .from("projects")
  .select("id,owner_id")
  .eq("name", "Harbour Centre");
if (pErr) throw pErr;
if (!projects?.length) {
  console.log("No 'Harbour Centre' projects found.");
  process.exit(0);
}
...
for (const project of projects) {
  for (const doc of DOCS) {
    ...
    const up = await supabase.storage
      .from("documents")
      .upload(path, bytes, { upsert: true, contentType: doc.file_type });
