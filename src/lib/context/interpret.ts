...
const METRIC_REGISTRY: { key: string; label: string; unit: Benchmark["unit"]; get: (v: EngineOutput["values"]) => number }[] = [
  ...
  { key: "development_spread", label: "Development Spread", unit: "bps", get: (v) => v.developmentSpreadBps },
  ...
  { key: "break_even_occupancy", label: "Break-even Occupancy", unit: "%", get: (v) => v.breakEvenOccupancyPct },
];

const IMPORTANCE: Record<string, number> = {
  dscr: 1.0, debt_yield: 0.92, development_spread: 0.9, equity_multiple: 0.85,
  profit_margin: 0.82, irr_estimate: 0.8, break_even_occupancy: 0.72,
  yield_on_cost: 0.7, loan_to_cost: 0.6, cash_on_cash: 0.5,
};

const BAND_WEIGHT: Record<InterpretationBand, number> = {
  critical: 1.0, weak: 0.92, soft: 0.6, in_line: 0.3, strong: 0.5, exceptional: 0.55, neutral: 0.2,
};
...
function fmtDelta(metricKey: string, delta: number, unit: Benchmark["unit"]): string {
  ...
  if (metricKey === "debt_yield" || metricKey === "yield_on_cost") return `${Math.round(a * 100)} bps`;
...
function contextNote(metricKey: string, band: InterpretationBand, ctx: DealContext): string | undefined {
  ...
  if (metricKey === "dscr" && ctx.loanStructure === "partial_io" && (band === "soft" || band === "weak")) {
    return `Coverage tightens after the ${Math.round(ctx.ioMonths / 12 * 10) / 10}-year interest-only period converts to amortizing.`;
  }
  if (metricKey === "break_even_occupancy" && (ctx.stage === "ground_up" || ctx.stage === "lease_up")) {
  ...
  if (metricKey === "development_spread" && (ctx.assetClass === "office" || ctx.assetClass === "retail")) {
...
export function interpretMetric(
  ...
  if (!benchmark) {
    return { metricKey, label, value, unit, band: "neutral", comparativePhrase: "", salience: 0, derived: [value] };
  }
  ...
  const salience = Math.round(Math.min(100, 100 * importance * (BAND_WEIGHT[band] * 0.7 + deviation * 0.5)));
  ...
  const a = Math.abs(delta);
  const displayedDelta = (metricKey === "debt_yield" || metricKey === "yield_on_cost") ? Math.round(a * 100)
    : unit === "bps" ? Math.round(a)
    : unit === "x" ? Math.round(a * 100) / 100
    : Math.round(a * 10) / 10;
  const derived = [value, benchmark.target, displayedDelta];
  return { metricKey, label, value, unit, band, benchmark, deltaToTarget: delta, comparativePhrase, contextNote: note, salience, derived };
}

export function interpretDeal(output: EngineOutput, ctx: DealContext, inputs?: BenchmarkInputs): Interpretation[] {
...
export function partitionInterpretations(items: Interpretation[]) {
  ...
  const concerns = items.filter((i) => i.band === "soft" || i.band === "weak" || i.band === "critical");
