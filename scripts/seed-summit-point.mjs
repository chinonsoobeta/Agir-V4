  ...
  /budget/i.test(n)
    ...
    : /rent_roll/i.test(n)
      ...
      : /appraisal/i.test(n)
        ...
        : /market/i.test(n)
          ? "Market Study"
          : /term_sheet|rate_lock/i.test(n)
            ? "Loan Package"
            : /lease/i.test(n)
              ? "Legal"
              : /environmental/i.test(n)
                ? "Other"
                : /tax/i.test(n)
                  ? "Other"
                  : /sponsor/i.test(n)
...
for (const f of files) {
  ...
  const up = await admin.storage
    .from("documents")
    .upload(storagePath, bytes, { contentType: TYPE(f), upsert: true });
  if (up.error) {
    console.error("upload failed", f, up.error.message);
    continue;
  }
  ...
  if (dErr) {
    console.error("doc row failed", f, dErr.message);
    continue;
  }
  n++;
}
