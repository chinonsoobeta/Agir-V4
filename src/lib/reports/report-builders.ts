...
export function buildReport(
  ...
  const governedData: ReportData = {
    ...data,
    assumptions: effectiveAssumptions(data.assumptions),
  };
  switch (reportType) {
    case "investor_report":
      return buildInvestorReport(governedData, opts);
    case "lender_package":
      return buildLenderPackage(governedData, opts);
    case "executive_summary":
      return buildExecutiveSummary(governedData, opts);
    case "internal_team_report":
      return buildInternalTeamReport(governedData, opts);
    default:
