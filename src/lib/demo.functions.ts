  ...
  list.push(
    {
      ...
      status: "approved",
      value: res.unit_count,
      source_doc: SPONSOR_DOC,
      source_text: `Residential rental units: ${res.unit_count} units`,
    },
    {
      key: "residential_rent_monthly",
      status: "approved",
      value: res.rent,
      source_doc: MARKET_DOC,
      source_text: `Residential rent: $${fmtMoney(res.rent)} per unit per month`,
    },
    {
      key: "residential_occupancy",
      status: "approved",
      value: res.occupancy_pct ?? null,
      source_doc: MARKET_DOC,
      source_text: `Residential occupancy: ${res.occupancy_pct}%`,
    },
    {
      key: "retail_sf",
      status: "approved",
      value: retail.avg_sf ?? null,
      source_doc: SPONSOR_DOC,
      source_text: `Retail area: ${fmtMoney(retail.avg_sf ?? 0)} SF`,
    },
    {
      key: "retail_rent_psf",
      status: "approved",
      value: retail.rent,
      source_doc: MARKET_DOC,
      source_text: `Retail rent: $${retail.rent} per square foot`,
    },
    {
      key: "retail_occupancy",
      status: "approved",
      value: retail.occupancy_pct ?? null,
      source_doc: MARKET_DOC,
      source_text: `Retail occupancy: ${retail.occupancy_pct}%`,
    },
    {
      key: "office_sf",
      status: "approved",
      value: office.avg_sf ?? null,
      source_doc: SPONSOR_DOC,
      source_text: `Office area: ${fmtMoney(office.avg_sf ?? 0)} SF`,
    },
    {
      key: "office_rent_psf",
      status: "approved",
      value: office.rent,
      source_doc: MARKET_DOC,
      source_text: `Office rent: $${office.rent} per square foot`,
    },
    {
      key: "office_occupancy",
      status: "approved",
      value: office.occupancy_pct ?? null,
      source_doc: MARKET_DOC,
      source_text: `Office occupancy: ${office.occupancy_pct}%`,
    },
  );
  ...
  list.push(
    {
      key: "debt_amount",
      status: "approved",
      value: scalar("loan_amount"),
      source_doc: LENDER_DOC,
      source_text: `Senior loan amount: $${fmtMoney(scalar("loan_amount") ?? 0)}`,
    },
    {
      key: "equity_amount",
      status: "approved",
      value: scalar("equity_amount"),
      source_doc: SPONSOR_DOC,
      source_text: `Common equity contribution: $${fmtMoney(scalar("equity_amount") ?? 0)}`,
    },
    {
      key: "interest_rate",
      status: "approved",
      value: scalar("interest_rate_pct"),
      source_doc: LENDER_DOC,
      source_text: `Interest rate: ${scalar("interest_rate_pct")}%`,
    },
    {
      key: "amortization_years",
      status: "approved",
      value: scalar("amort_years"),
      source_doc: LENDER_DOC,
      source_text: `Amortization: ${scalar("amort_years")}-year`,
    },
    {
      key: "min_dscr",
      status: "approved",
      value: scalar("min_dscr"),
      source_doc: LENDER_DOC,
      source_text: `Minimum DSCR covenant: ${scalar("min_dscr")?.toFixed(2)}x`,
    },
    {
      key: "lender_stabilized_occupancy",
      status: "approved",
      value: scalar("lender_stabilized_occupancy_pct"),
      source_doc: LENDER_DOC,
      source_text: `Lender stabilization requirement: ${scalar("lender_stabilized_occupancy_pct")}%`,
    },
    {
      key: "rent_growth",
      status: "approved",
      value: scalar("rent_growth_pct"),
      source_doc: MARKET_DOC,
      source_text: `Annual rent growth: ${scalar("rent_growth_pct")}%`,
    },
  );
...
export const seedHarbourCentre = createServerFn({ method: "POST" })
  ...
  .handler(async ({ context }) => {
    ...
    const uploadedPaths: Record<
      string,
      { path: string; size: number; type: string; category: string }
    > = {};
    ...
    const { data: project, error } = await context.supabase
      ...
      .insert({
        ...
        interest_rate: 6.25,
        notes:
          "Demo deal: 220-unit residential tower over 18k SF retail and 32k SF office. Exit cap is conflicted (4.75% broker vs 5.25% lender); expense ratio, hold period and selling costs are absent from the documents.",
      })
      .select()
      .single();
    ...
    const assumptionRows = buildSeedAssumptions().map((a) => {
      ...
      const confidence =
        a.status === "approved" || a.status === "calculated"
          ? 100
      ...
      return {
        ...
        owner_id: context.userId,
        field_key: a.key,
        field_label: def?.label ?? a.key,
        category: def?.category ?? "Costs",
        unit: def?.unit ?? "$",
        ...
        ai_reasoning:
          a.status === "conflicting"
            ? "Two documented values disagree; resolve before underwriting (never averaged)."
            : a.status === "calculated"
              ? "Calculated deterministically from the five extracted budget lines."
              : a.status === "missing"
                ? "Not found in any document; accept a default to proceed."
