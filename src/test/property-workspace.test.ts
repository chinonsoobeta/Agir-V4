...
describe("canonical property workspace backend contract", () => {
  ...
  test("exports the stable server-function surface used by Property routes", () => {
    ...
    expect(functions).toContain('rpc("search_properties_page"');
    expect(functions).not.toContain('rpc("create_property_search_session"');
    expect(functions).not.toContain('rpc("get_property_search_session_page"');
