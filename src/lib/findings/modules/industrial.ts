...
export function industrialFindings(input: NormalizedFindingsInput): Finding[] {
  ...
  const isIndustrial =
    revenue.some((r) =>
      /warehouse|cold|storage|logistics|flex|distribution|industrial/i.test(r.unitType),
    ) ||
  ...
  if (concentration != null) {
    ...
    const gpr = revenue.map((r) => ({
      ...
      gpr:
        r.rentBasis === "per_sf"
          ? r.unitCount * Number(r.avgSf ?? 0) * r.rent
          : r.unitCount * r.rent * 12,
    }));
    ...
    concentrationEvidence =
      concentration != null ? `${top.name} contributes ${pct(concentration)} of gross rent` : "";
  } else {
    concentrationEvidence = "";
  }
  ...
  const coldComponent =
    revenue.find((r) => COLD_RE.test(r.unitType)) ||
  ...
  const interestRow = input.assumptions.find(
    (a) =>
      a.field_key === "interest_rate" &&
      (RATE_LOCK_RE.test(a.source_location ?? "") || RATE_LOCK_RE.test(a.source_text ?? "")),
  );
  if (interestRow) {
    findings.push(
      f(
        ...
        interestRow.value_numeric != null
          ? { interest_rate_pct: Number(interestRow.value_numeric) }
          : {},
