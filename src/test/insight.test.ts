...
import { runUnderwriting, mapleHeightsInput, buildAllowedValues, verifyNumericProvenance, type UnderwritingInput } from "@/lib/engine";
import {
  buildInsight,
  writeNarrative,
  deriveDealContext,
...
function mixedUseInput(): UnderwritingInput {
  return {
    budget: { land: 26_000_000, hard: 175_000_000, soft: 24_000_000, contingency: 10_000_000, financingInterest: 15_000_000 },
    revenueProgram: [
      { unitType: "Residential", unitCount: 320, avgSf: 850, rent: 3_900, rentBasis: "per_unit", occupancyPct: 95 },
      { unitType: "Retail", unitCount: 1, avgSf: 45_000, rent: 48, rentBasis: "per_sf", occupancyPct: 93 },
      { unitType: "Office", unitCount: 1, avgSf: 80_000, rent: 55, rentBasis: "per_sf", occupancyPct: 90 },
      { unitType: "Last-Mile Flex", unitCount: 1, avgSf: 60_000, rent: 22, rentBasis: "per_sf", occupancyPct: 96 },
    ],
    constructionMonths: 26, leaseUpMonths: 18, stabilizedOccupancyPct: 93, expenseRatioPct: 31,
    otherIncomeAnnual: 1_200_000, exitCapRatePct: 5.0, loanAmount: 150_000_000, interestRatePct: 6.0,
    amortYears: 30, ioMonths: 30, avgOutstandingFactor: 0.6, sellingCostsPct: 1.5, holdYears: 7,
    equityAmount: 100_000_000, rentGrowthPct: 3.0, expenseGrowthPct: 2.5,
  };
}
...
describe("insight layer: deal context", () => {
  test("classifies asset class, loan structure and stage from data alone", () => {
    ...
    const mu = deriveDealContext(mixedUseInput(), { type: "mixed_use", location: "Gateway District" });
...
describe("insight layer: benchmarks", () => {
  test("context-aware and carry provenance, with firm + portfolio layering", () => {
    ...
    const firm = resolveBenchmark("debt_yield", ctx, { firmConfig: { overrides: { multifamily: { debt_yield: { target: 10 } } } } });
    expect(firm?.target).toBe(10);
    expect(firm?.source).toBe("firm");

    const port = resolveBenchmark("debt_yield", ctx, { portfolioNorms: { sampleSize: 12, bands: { debt_yield: { p25: 8, p50: 10, p75: 12, n: 12 } } }, portfolioMinSample: 8 });
...
describe("insight layer: synthesis", () => {
  test("thesis + bullets are produced, deterministic, and the narrative is provenance-clean", () => {
    ...
    const text = [b1.thesis, writeNarrative(b1, "ic"), writeNarrative(b1, "lender"), writeNarrative(b1, "investor"), ...b1.bullets].join("  ");
    const allowed = buildAllowedValues(
      Object.values(out.values),
      b1.derivedValues,
      [input.loanAmount, input.exitCapRatePct, input.equityAmount ?? 0, input.interestRatePct],
    );
