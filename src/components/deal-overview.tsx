...
import { TrendingUp, AlertTriangle, Lightbulb, ListChecks, ArrowUpRight, ArrowDownRight, Quote } from "lucide-react";

const SEV_TONE: Record<string, keyof typeof TONE_CHIP> = {
  critical: "reject", high: "reject", medium: "condition", low: "neutral",
};
    ...
    return (
      <Card className="p-12 text-center elevated">
        ...
        <p className="text-sm text-muted-foreground mt-2 max-w-md mx-auto">
          This deal has not been underwritten. Approve the required assumptions in the <strong>Assumptions</strong> tab,
          then run the deterministic engine in <strong>Analysis</strong>. The Investment Score, findings and recommendation will appear here.
        </p>
      </Card>
    );
  }

  const primaryReason = f?.recommendationFindings?.[0]?.rationale
    ...
    <div className="space-y-5">
      ...
      <Card className="p-6 elevated">
        ...
        <div className="grid md:grid-cols-2 gap-3 mt-5">
          ...
          <SummaryBox icon={Lightbulb} tone="approve" label="Top Opportunity"
            title={topOpp?.title ?? "No standout opportunity identified"} body={topOpp?.rationale} />
        </div>
      </Card>
      ...
      <div className="grid lg:grid-cols-2 gap-4">
        ...
        <FindingsColumn title="Opportunities" icon={Lightbulb} tone="return" findings={f?.opportunities ?? []} empty="No opportunities surfaced." />
        <FindingsColumn title="Approval Conditions" icon={ListChecks} tone="condition" findings={f?.approvalConditions ?? []} empty="No conditions required." />
      </div>
      ...
      <div className="grid lg:grid-cols-2 gap-4">
        ...
        <Card className="p-6 elevated">
          ...
          <div className="grid grid-cols-2 gap-5 mt-3">
            <DriverList title="Value drivers" icon={ArrowUpRight} tone="approve" drivers={f?.primaryDrivers ?? []} />
            <DriverList title="Risk drivers" icon={ArrowDownRight} tone="reject" drivers={f?.downsideDrivers ?? []} />
          </div>
        </Card>
      </div>
...
function SummaryBox({ icon: Icon, tone, label, title, body }: { icon: any; tone: keyof typeof TONE_CHIP; label: string; title: string; body?: string }) {
  return (
    <div className={`rounded-lg border p-4 ${TONE_CHIP[tone].replace(/text-\S+/, "")} border-border bg-muted/20`}>
      <div className="flex items-center gap-2">
...
function FindingsColumn({ title, icon: Icon, tone, findings, empty }: { title: string; icon: any; tone: keyof typeof TONE_CHIP; findings: Finding[]; empty: string }) {
  return (
      ...
      {findings.length === 0 ? (
        ...
        <ul className="mt-3 space-y-3">
          {findings.slice(0, 5).map((x) => (
            <li key={x.id} className="border-l-2 pl-3" style={{ borderColor: `var(--color-${tone === "approve" ? "success" : tone === "reject" ? "destructive" : tone === "condition" ? "warning" : "chart-2"})` }}>
              <div className="flex items-center gap-2">
                <span className="text-sm font-medium">{x.title}</span>
                <span className={`text-[9px] uppercase tracking-wider px-1.5 py-0.5 rounded border ${TONE_CHIP[SEV_TONE[x.severity] ?? "neutral"]}`}>{x.severity}</span>
              </div>
...
function DriverList({ title, icon: Icon, tone, drivers }: { title: string; icon: any; tone: keyof typeof TONE_CHIP; drivers: { rank: number; name: string; rationale: string }[] }) {
  return (
    <div>
      ...
      <ol className="mt-2.5 space-y-2">
        {drivers.slice(0, 5).map((d) => (
          <li key={d.rank} className="flex gap-2 text-xs">
            ...
            <span className="font-medium" title={d.rationale}>{d.name}</span>
          </li>
        ))}
