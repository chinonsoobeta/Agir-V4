...
function multiWb(sheets: { name: string; aoa: unknown[][] }[]): ArrayBuffer {
  const book = XLSX.utils.book_new();
  for (const s of sheets) XLSX.utils.book_append_sheet(book, XLSX.utils.aoa_to_sheet(s.aoa), s.name);
  return XLSX.write(book, { type: "array", bookType: "xlsx" }) as ArrayBuffer;
}
...
describe("2B: multi-sheet workbooks", () => {
  test("budget: scans every sheet and selects the budget tab, not the first sheet", () => {
    const { inserted, meta } = parseBudgetWorkbook(multiWb([
      { name: "Cover", aoa: [["Project", "Harbour Centre"], ["Prepared by", "Capital Markets"]] },
      { name: "Rent Roll", aoa: [["Unit Type", "Units", "Rent"], ["1BR", 60, 2200]] },
      { name: "Development Budget", aoa: [["Category", "Line Item", "Amount"], ["land", "Land", 34_500_000], ["hard", "Hard costs", 162_000_000]] },
    ]));
  ...
  test("rent roll: parses multiple rent-roll tabs and merges their rows", () => {
    const { inserted, meta } = parseRentRollWorkbook(multiWb([
      { name: "Summary", aoa: [["Metric", "Value"], ["NOI", 6_000_000]] },
      { name: "Residential", aoa: [["Unit Type", "Units", "Rent", "Occupancy"], ["1BR", 60, 2200, 95], ["2BR", 50, 2600, 95]] },
      { name: "Commercial", aoa: [["Component", "Units", "Rent PSF", "Rentable SF", "Occupancy"], ["Retail", 1, 42, 18_000, 92]] },
    ]));
...
describe("2C: merged cells", () => {
  test("budget: a vertically merged category label is propagated to every spanned row", () => {
    ...
    const { inserted, meta } = parseBudgetWorkbook(wbWithMerges(aoa, [{ s: { r: 1, c: 0 }, e: { r: 2, c: 0 } }]));
  ...
  test("budget: a merged title banner above the header is skipped and its scale honored", () => {
    ...
    const { inserted, meta } = parseBudgetWorkbook(wbWithMerges(aoa, [{ s: { r: 0, c: 0 }, e: { r: 0, c: 2 } }]));
