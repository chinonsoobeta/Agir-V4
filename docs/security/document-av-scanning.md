...
2. **External AV scan (mandatory in staging/production).** When `DOCUMENT_SCAN_URL` is set, the
...
- **Fail-closed is the default.** If the scanner is down, uploads are rejected
  with "Safety scan unavailable". `DOCUMENT_SCAN_FAIL_OPEN=1` works only in
  development/demo/test; it is impossible in staging/production.
- `npm run env:validate` fails when `DOCUMENT_SCAN_URL` is unset in
