...
export function buildLenderPackage(data: ReportData, opts: { generatedLabel: string }): MemoReport {
  ...
  const loanSummary: string[][] = [
    ...
    [
      "Loan payoff at exit",
      oVal("base", "loan_payoff_at_exit") == null
        ? "Not available"
        : money(oVal("base", "loan_payoff_at_exit")!),
    ],
  ];
  sections.push({
    heading: "Loan Summary",
    table: { columns: ["Term", "Value"], rows: loanSummary },
  });
  ...
  const su: string[][] = [
    ...
    ...(Math.abs(core.fundingGap) > 1
      ? [["Funding gap (uncommitted)", money(core.fundingGap)]]
      : []),
  ...
  if (core.minDscr != null && core.dscr != null) {
    cov.push([
      "Minimum DSCR",
      `Required ${x(core.minDscr)}`,
      x(core.dscr),
      core.dscr >= core.minDscr ? "PASS" : "BREACH",
    ]);
  }
  if (requiredNoi != null && core.noi != null) {
    cov.push([
      "Required vs actual NOI",
      `Required ${money(requiredNoi)}`,
      money(core.noi),
      core.noi >= requiredNoi ? "PASS" : "SHORT",
    ]);
  }
  if (core.ltc != null)
    cov.push([
      "Loan-to-cost",
      `Loan ${money(core.loan)} / TDC ${money(core.tdc)}`,
      pct(core.ltc),
      "INFO",
    ]);
  for (const r of data.revenue) {
    ...
    if (occ != null && core.lenderOcc != null) {
      cov.push([
        `Occupancy: ${r.unit_type}`,
        `Requirement ${pct(core.lenderOcc)}`,
        pct(occ),
        occ >= core.lenderOcc ? "PASS" : "BELOW",
      ]);
    }
  }
  if (cov.length)
    sections.push({
      ...
      table: { columns: ["Covenant", "Basis", "Underwritten", "Status"], rows: cov },
    });

  // 4. Debt service
  const ioDscrRow = data.outputs.find(
    (o) => o.scenario_key === "base" && o.metric_key === "interest_only_dscr",
  );
  const debt: string[][] = [
    ["Annual debt service", ads == null ? "Not available" : money(ads)],
    ...(ioDscrRow
      ? [["Interest-only DSCR (secondary)", x(Number(ioDscrRow.value_numeric ?? 0))]]
      : []),
    [
      "Loan payoff at exit",
      oVal("base", "loan_payoff_at_exit") == null
  ...
  if (scenarioOrder.length > 1) {
    const metrics: Array<[string, string]> = [
      ...
      ["Loan payoff", "loan_payoff_at_exit"],
    ];
    ...
    sections.push({
      heading: "Stress-Case Debt Metrics",
      table: { columns: ["Metric", ...scenarioOrder.map((s) => SCEN_LABELS[s] ?? s)], rows },
    });
  }
  ...
  if (core.dscr != null && core.minDscr != null && core.dscr < core.minDscr)
    riskBullets.push("- Credit risk: underwritten DSCR is below the covenant minimum.");
  if (Math.abs(core.fundingGap) > 1)
    riskBullets.push("- Funding gap risk: committed equity is short of required equity.");
  if (data.flags.some((f) => String(f.check_key).startsWith("occupancy_vs_lender")))
    riskBullets.push(
      "- Stabilization risk: one or more components are below the lender occupancy requirement.",
    );
  riskBullets.push(
    "- Exit/refinance risk: takeout depends on the exit cap and stabilized NOI shown above.",
  );
  ...
  const notReady =
    (core.dscr != null && core.minDscr != null && core.dscr < core.minDscr) ||
    Math.abs(core.fundingGap) > 1;
