...
const HARBOUR_ASSUMPTIONS = [
  ...
  ["min_dscr", 1.2],
  ["lender_stabilized_occupancy", 93],
  ["rent_growth", 3],
...
function buildContext() {
  ...
  const verdict = computeInvestmentVerdict({
    ...
    stress_dscr: SCEN[2].out.values.dscr,
    stress_equity_multiple: SCEN[2].out.values.equityMultiple,
...
describe("Deterministic memo template", () => {
  test("produces all required sections, non-empty", () => {
    ...
    const memo = buildDeterministicMemo({
      ...
      assumptions: HARBOUR_ASSUMPTIONS,
      engineInputs: [],
      outputs,
      cashFlows,
      flags,
  ...
  test("never invents a number: zero provenance orphans", () => {
    ...
    const memoText = Object.values(memo)
